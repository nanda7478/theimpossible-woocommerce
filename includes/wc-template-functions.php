...
function wc_get_gallery_image_html( $attachment_id, $main_image = false ) {
	...
	return '<div data-thumb="' . esc_url( $full_src[0] ) . '" class="woocommerce-product-gallery__image"><img src="'. esc_url( $full_src[0] ) .'"></div>';
}
...
if ( ! function_exists( 'woocommerce_checkout_payment' ) ) {
	...
	if($_SESSION["language"] == 'ja'){
		...
		wc_get_template( 'checkout/payment.php', array(
			...
			'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( $place, 'woocommerce' ) ),
		) );
	}
}
...
function wc_get_star_rating_html( $rating, $count = 0 ) {
	$html = '<span style="width:' . ( ( $rating * 16.80 )  ) . 'px">';
    if( is_main_site() ){
	if ( 0 < $count ) {
		/* translators: 1: rating 2: rating count */
		$html .= sprintf( _n( 'Rated %1$s out of 5', 'Rated %1$s out of 5', $count, 'woocommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>', '<span class="rating">' . esc_html( $count ) . '</span>' );
	} else {
		/* translators: %s: rating */
    	...
    	if ( 0 < $count ) {
		/* translators: 1: rating 2: rating count */
		$html .= sprintf( _n( '定格 %1$s のうち 5', '定格 %1$s out of 5', $count, 'woocommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>', '<span class="rating">' . esc_html( $count ) . '</span>' );
	} else {
		/* translators: %s: rating */
		$html .= sprintf( esc_html__( '定格 %s のうち 5', 'woocommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
	}
    }
