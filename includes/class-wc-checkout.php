...
class WC_Checkout {
	...
	public function __wakeup() {
		...
		wc_doing_it_wrong( __FUNCTION__, __( 'Unserializing instances of this class is forbidden.', 'woocommerce' ), '2.1' );
	   	...
	   	wc_doing_it_wrong( __FUNCTION__, __( 'このクラスのインスタンスの直列化解除は禁止されています.', 'woocommerce' ), '2.1' );
	   } 
	}
			...
			foreach ( $fieldset as $key => $field ) {
				...
				switch ( $fieldset_key ) {
					case 'shipping':
					    ...
					    $field_label = sprintf( __( '運送 %s', 'woocommerce' ), $field_label );	
					    }
						break;
					case 'billing':
						/* translators: %s: field name */
						...
						$field_label = sprintf( __( '課金 %s', 'woocommerce' ), $field_label );
						}
						break;
				}
					...
					if ( $validate_fieldset && '' !== $data[ $key ] && ! WC_Validation::is_postcode( $data[ $key ], $country ) ) {
						switch ( $country ) {
							...
							default:
								...
								$postcode_validation_notice = sprintf( __( '%s 有効ではありません 郵便番号 / 郵便番号。', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' );
							}
						}
				...
				if ( in_array( 'phone', $format, true ) ) {
					if ( $validate_fieldset && '' !== $data[ $key ] && ! WC_Validation::is_phone( $data[ $key ] ) ) {
						...
						$errors->add( 'validation', sprintf( __( '%s 有効な電話番号ではありません。', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
					}
					}
				}
				...
				if ( in_array( 'email', $format, true ) && '' !== $data[ $key ] ) {
					...
					if ( $validate_fieldset && ! $email_is_valid ) {
						...
						$errors->add( 'validation', sprintf( __( '%s is not a valid email address.', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
							...
							$errors->add( 'validation', sprintf( __( '%s 有効なメールアドレスではありません。', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
						}
						continue;
					}
				}
				...
				if ( $validate_fieldset && $required && '' === $data[ $key ] ) {
                  ...
                  //  $errors->add( 'required-field', apply_filters( 'woocommerce_checkout_required_field_notice', sprintf( __( '%s は必須フィールドです.', 'woocommerce' ), '<strong>' . esc_html( $field_label ) . '</strong>' ), $field_label ) );
	...
	protected function validate_checkout( &$data, &$errors ) {
		...
		if ( empty( $data['woocommerce_checkout_update_totals'] ) && empty( $data['terms'] ) && ! empty( $_POST['terms-field'] ) ) { // WPCS: input var ok, CSRF ok.
			...
			if( is_main_site() ){
				$errors->add( 'terms', __( 'Please read and accept the terms and conditions to proceed with your order.', 'woocommerce' ) );
				     ...
				     $errors->add( 'terms', __( 'ご注文を進めるには、利用規約を読んで同意してください。', 'woocommerce' ) );
			    	}
		...
		if ( WC()->cart->needs_shipping() ) {
			...
			if ( empty( $shipping_country ) ) {
				...
				if($_SESSION["language"] == 'ja'){
                  $errors->add( 'shipping', sprintf( __( 'Unfortunately <strong>we do not ship %s</strong>. 別の配送先住所を入力してください。', 'woocommerce' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->customer->get_shipping_country() ) );
				...
				foreach ( WC()->shipping->get_packages() as $i => $package ) {
			     ...
			     $errors->add( 'shipping', __( '配送方法が選択されていません。住所を再確認するか、サポートが必要な場合はお問い合わせください。', 'woocommerce' ) );
			    	}
