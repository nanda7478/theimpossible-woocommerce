	...
	function wc_create_new_customer( $email, $username = '', $password = '' ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			...
			return new WP_Error( 'registration-error-invalid-email', __( 'Please provide a valid email address.', 'woocommerce' ) );
				...
				return new WP_Error( 'registration-error-invalid-email', __( '有効なメールアドレスを入力してください。', 'woocommerce' ) );
			}
		}
		...
		if ( email_exists( $email ) ) {
			...
			return new WP_Error( 'registration-error-email-exists', apply_filters( 'woocommerce_registration_error_email_exists', __( 'An account is already registered with your email address. Please log in.', 'woocommerce' ), $email ) );
		  	...
		  	return new WP_Error( 'registration-error-email-exists', apply_filters( 'woocommerce_registration_error_email_exists', __( 'Aアカウントはすでにあなたのメールアドレスに登録されています。ログインしてください。', 'woocommerce' ), $email ) );
		  }
		}
		...
		if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) || ! empty( $username ) ) {
			...
			if ( empty( $username ) || ! validate_username( $username ) ) {
				...
				return new WP_Error( 'registration-error-invalid-username', __( 'Please enter a valid account username.', 'woocommerce' ) );
			   	...
			   	return new WP_Error( 'registration-error-invalid-username', __( '有効なアカウントのユーザー名を入力してください。', 'woocommerce' ) );
			   }
			}
			...
			if ( username_exists( $username ) ) {
				...
				return new WP_Error( 'registration-error-username-exists', __( 'An account is already registered with that username. Please choose another.', 'woocommerce' ) );
			   	...
			   	return new WP_Error( 'registration-error-username-exists', __( 'アカウントはすでにそのユーザー名で登録されています。別のものを選択してください。', 'woocommerce' ) );
			   }
			}
		} else {
		...
		if ( empty( $password ) ) {
			...
			return new WP_Error( 'registration-error-missing-password', __( 'Please enter an account password.', 'woocommerce' ) );
		   	...
		   	return new WP_Error( 'registration-error-missing-password', __( 'アカウントのパスワードを入力してください。', 'woocommerce' ) );
		   }
		}
